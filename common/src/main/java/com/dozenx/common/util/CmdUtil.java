...
public class CmdUtil {
    ...
    public static String execCommand(String commandStr,String filePath)throws IOException{
        try {
            logger.debug("在"+filePath+"下执行命令"+commandStr);
    ...
    public static void killLike(String content,String path){
        String commandStr= "ps -ef | grep "+content;
        String result = null;
        try {
            result = execCommand( commandStr, path);
        } catch (IOException e) {
            e.printStackTrace();
        }
        logger.debug(commandStr+"==>command return:  "+result);
        String[] lines = result.split("\n");
        for(String s :lines){
            StringTokenizer st=new StringTokenizer(s);
            logger.debug("command return line "+s);
            st.nextToken();
            String pid =st.nextToken();
            try {
                execCommand("kill -9 "+pid,path);
            } catch (IOException e) {
                e.printStackTrace();
            }
