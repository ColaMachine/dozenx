<template>
  <div>
      <div class="location">
       <Select v-model="locationValue.provinceId" @on-change="getCity" placeholder="省" clearable style="width:180px" ref="mySelector" :disabled="readonly" >
            <Option v-for="item in provinceIdOption" :value="item.id+'-'+item.areaName" :key="item.id">{{ item.areaName }}</Option>
        </Select>
        <Select v-model="locationValue.cityId" placeholder="市" clearable style="width:180px" :disabled="readonly" >
      ...
      addLocation(){
          ...
          if(this.locationValue.provinceId){
              ...
              this.d_value.forEach(function(element){
                 if(element.provinceName==province.split('-')[1]&&element.cityName==city.split('-')[1]){
                    _this.flag=false
                    _this.$Notice.error({
                        title:'不可以重复添加'
                    })
