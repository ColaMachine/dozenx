<template>
  <div>
    <ul class="search-bar">
      <li class="title">关联底库</li>
      <li class="org-add">
        <Button class="search-btn" @click="add">关联底库</Button>
      </li>
    </ul>
...
<script>
  ...
  export default {
    ...
    methods: {
      getTransfer(){
        $http.get('/api/mssrv/sys/auth/org/group/notin/list', {
          params: {
            params:{}
          }
        }).then((res) => {
      ...
      getTransferChoice(){
        $http.get('/api/mssrv/sys/auth/org/group/in/list', {
          params: {
            params:{
      ...
      search(curPage) {
        ...
        $http.get('/api/mssrv/sys/auth/org/group/list', {
          params: {
      ...
      submit() {
        ...
        //   return
        // }
        $http.put('/api/mssrv/sys/auth/org/group/relation',{orgId:this.searchData.orgId,groupIds:this.targetKeys} ).then(() => {
          this.$Notice.success({
            title: "提交成功"
          });
          this.hideFormModal();
