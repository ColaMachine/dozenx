       ...
       <div class="video">
         ...
         <main>
           <Row :gutter="16">
             ...
             <!-- <iframe ref="myframe" style="width:100%;height:350px" class="video-js vjs-live stream" :src="'/api/mssrv/camera/jpg/show/'+currentVideo.id"   name="myframe"  frameborder="0" scrolling="auto" width="100%" height="100%"  ></iframe> -->
                <!-- <video ref='videoPlayer' class="video-js vjs-live stream" controls></video> -->
             </Col>
           </Row>
         </main>
       </div>
...
<script>
  ...
  export default {
    ...
    mounted(){
   var _this=this;
       this.imageFreshIntervalId =  setInterval(()=>{

            var image =new Image();
            image.src="http://192.168.213.91:2019/mssrv/static/data/capture/"+_this.currentVideo.id+"/now.jpg?t="+Date.parse(new Date());
            image.onload=function(){
                //_this.nowImage = image.src;
                var canvas = document.getElementById("mycanvas");

                if(!canvas){
                console.log("canvas 不存在 已经注销定时任务");
                  clearInterval(_this.imageFreshIntervalId);
                  return ;
                }
    ...
    methods:{
      clearSocket(){
       ...
       // clearInterval(this.imageFreshIntervalId)
        ...
        if (typeof e.data == 'string') {
            try {
                ...
                if(typeof data == 'object'  ){
                  ...
                  console.log("compareData.length:"+this.compareData.length);
