...
public class ZanController extends BaseController{
    /** 日志 **/
    private Logger logger = LoggerFactory.getLogger(ZanController.class);
    /** 权限service **/
    @Autowired
    ...
    public ResultDTO listAll(HttpServletRequest request) {
        ...
        List<Zan> zans = zanService.listByParams(params);
        ...
        */
        ...
        if(!StringUtil.isBlank(createTime)){
            if(StringUtil.checkNumeric(createTime)){
                ...
                zan.setCreateTime(new Timestamp( DateUtil.parseToDate(createTime, "yyyy-MM-dd HH:mm:ss").getTime()));
            }
        }
    ...
    public ResultDTO exportExcel(HttpServletRequest request){
        ...
        // 查询list集合
        List<Zan> list =zanService.listByParams(params);
        // 存放临时文件
