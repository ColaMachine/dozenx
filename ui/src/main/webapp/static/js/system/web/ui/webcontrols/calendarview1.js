...
CalendarView.prototype.render = function() {
  ...
  str = "<div id=\"mask\" onClick=\"Instance('" +
    ...
    "<div>" +
    "<div>" +
    "<div  ><button onClick=\"window.location='/home/sys/auth/login.htm'\">登录</button>" +
    ...
    this.index + "').changeToWeekView()\">星期</a>" +
  ...
  "<a style='display:none' id='ca-ty-month' onClick=\"Instance('" + this.index +
    ...
    "</div>" + "</div>" + "</div>" +
    "<div>" + "<div  class='ls-hidden' style=\"padding-top:1px\">" +
...
CalendarView.prototype.getCalendarEventDialogView = function() {
  var str = "<div onClick=\"try{event.stopPropagation();}catch(e){event.cancelBubble=true;}\" id=\"calendarEventDialog\" class=\"calendarEventDialog\" style=\"display:none\"><table>" +
    ...
    this.index +
    "').editCalendarEventAction()\"></input><select id='event_type'><option value=0>工作</option><option value=1>生活</option><option value=2>学习</option><option value=9>考勤</option><option value=11>计划</option><option value=12>正在做</option><option value=13>带校验</option><option value=14>已完成</option></select>" +
    "</td></tr>" +
    "</table></div>";
  return str;
};
...
CalendarView.prototype.getListWeekView = function() {
  ...
  var _date = 1;
  var _rows = 1;
  var str = "<div id=\"scrolltimedeventswk\" style='border:1px solid red;width:100%;height:100%'></div>";
...
CalendarView.prototype.displaySingleEvent = function(ce) {
  ...
  div_html.attachEvent("onmousedown", function(event) {
...
CalendarView.prototype.createEventAction = function(e) { // 分解此方法
  ...
  var startTime = parseDate(ce.endDay + " " + ce.startTimeSV + ":00", "yyyy-MM-dd HH:mm:ss");
  ce.startTime = parseInt(startTime.getTime() / 60000);
  var endTime = parseDate(ce.endDay + " " + ce.endTimeSV + ":00", "yyyy-MM-dd HH:mm:ss");
...
function judgeIfNeed2Save(ceid) {
  var ce = ca.getCalendarEvent(ceid);
  console.log("startTime:"+ getTimes(ce.startDay, ce.startTimeSV));
  ...
  var newObj ={
      ...
      "isdel":ce.isdel,
      "title":ce.title,
      "startTime":getTimes(ce.startDay, ce.startTimeSV),
        "endTime":getTimes(ce.endDay, ce.endTimeSV),
...
activity.type=ce.type;
  ...
  if(ce.startDay) {
    activity.startTime = getTimes(ce.startDay, ce.startTimeSV);
    //	activity. ENDTIME = getTimes(ce.startDay,ce.startTimeSV);
  }
