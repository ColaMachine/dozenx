...
var _LOGIN_TYPE = '{@loginType@}';/**登录类型: authed 代表已认证过，控制页面显示“免费上网”；unauth 代表未认证过，页面显示 “手机号认证”或“用户名认证”*/
...
var ComponentClass = function () {
    ...
    function addHeader(cpt_param) {
        var head_files = [],
            ...
            component_css = path + '/css/component.css',
            setting_css = path + '/css/setting.css',
            entity_js = path + '/script/Entity.js',
            setting_js = path + '/script/Setting.js';
    ...
    function createReactEntity(data) {
        ...
        var htm = '<li data-unique="'+compJson.canUnique+'" data-classify="'+compJson.classify+'" data-id="'+componentId+'" data-code="'+compJson.code+'" data-set-code="'+compJson.setCode+'" '+
            ...
            '<div id="'+comp_id+'"></div>'+
            '<script type="text/javascript">var '+comp_id+' = '+compJson.code+'(null, "'+comp_id+'").render(); '+comp_id+'.setState('+JSON.stringify(data.json)+')</script></li>';
    ...
    function init() {
        ...
        if (location.pathname.indexOf('test.html') == -1) {
            // TODO 加载所有组件数据
     ...
     * 同类组件实体只能拖拉一个
     */
    function entityUnique(classify, action) {
        action = action || 'disable';
        // 禁用拖拉
        if (action == 'disable') {
            $('#comp-list li[data-classify="'+classify+'"][data-unique="1"]').draggable('disable')
                .css('cursor', 'not-allowed');
        } else {
            $('#comp-list li[data-classify="'+classify+'"][data-unique="1"]').draggable('enable')
                .css('cursor', '');
        }
    }
...
function entityInitFault() {
    var entity_htm_fault = '<li style="min-height:auto"><div class="entity-cover"></div><div class="entity-del-fault"></div><p class="entity-fault">该组件存在内部错误，请删除</p></div></li>';
...
function componentSortable(pageId) {
    ...
    $("#comp-main").sortable({
        ...
        stop: function (e, ui) {
            ...
            if (el.attr('data-unique') == 1) {
                compClass.entityUnique(componentJson.classify);
            }
        }
    });
    ...
    for (var i=0,len=pageEntitys.length; i<len; i++) {
        ...
        if (compJson.canUnique == 1 && item.componentOperation != 'remove') {
            compClass.entityUnique(compJson.classify);
        }
    }
}
...
$('.entities').on('click', 'li .entity-del', function(e){
    ...
    // 判断当前组件是否唯一
    if (el.attr('data-unique') == 1) {
        compClass.entityUnique(compJson.classify, 'enable');
    }
    $('#entity-checkbox').hide();
});
