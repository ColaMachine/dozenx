...
public class BaseController extends ResultAction {
    ...
    @Autowired
    protected ThirdUserService thirdUserService;
...
//        HttpSession session = request.getSession();
    ...
    public SessionUser getUser(HttpServletRequest request) {
        //todo
        //入参request，返回体封装成sessionUser返回
        request.getHeader("access_token");
        String accessToken = getSessionId(request);

        SessionUser sessionUser =  this.getSessionAttribute(request,Constants.SESSION_USER,SessionUser.class);

        if(sessionUser !=null)
            return sessionUser;

        if(thirdUserService !=null){
            return thirdUserService.getUser(request);
        }
...
//
    ...
    /**
     * tianyi kanjia 的应用说要保持app登录时间30天 所以 让调用者自定义时间
     * @param request
     * @param key
     * @param object
     * @param timeInMills
     */
    public void setSessionAttribute(HttpServletRequest request, String key, Object object,int timeInMills) {
        if (object == null) {
            return;
        }
        //先往session里面存放 再在redis中存放
        ...
        String sessionKey = getSessionId(request);
        logger.info("redis setEx" + sessionKey + "_" + key);
        RedisUtil.setByteAry(sessionKey + "_" + key, toByteArray(object), timeInMills);
    }
