...
public class TokenHttpUtil {
    ...
    public static String urlResetAccessToken(String url){
        if(StringUtils.isBlank(url)){
            throw new ValidException("E0000002", MessageUtil.getMessage("E0000002", "接口地址"));//{0}不允许为空!
        }
        StringBuilder newURL = new StringBuilder(url);//生成的新的url，自动拼接access_token
        if(url.indexOf("?") == -1){//不包含?时
    ...
    public static TokenHttpResult analysisPriResult(HttpResult httpResult, String path, String params){
        ...
        /* 403  Forbidden  资源不可用。服务器理解客户的请求，但拒绝处理它，通常由于服务器上文件或目录的权限设置导致。  */
            ...
            if(StringUtils.isBlank(errorMessage)){
                throw new InterfaceException(MessageUtil.getMessage("E0000002", "errorMessage"), path, params, result);//{0}不允许为空!
            }
        ...
        /* 其它状态码  */
            ...
            throw new InterfaceException(MessageUtil.getMessage("E2000018"), path, params, result);//接口异常!
        }
    }
