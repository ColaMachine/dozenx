...
//                  , dataType = DataType.INTEGER, in=InType.path,required = true),
    ...
    private SysUser getInfoFromMap(Map<String,Object> bodyParam) throws Exception {
        ...
        ValidateUtil vu = new ValidateUtil();
        String validStr="";
        vu.add("username", sysUser.getUsername(), "用户名",  new Rule[]{new Length(15),new Regex(RegexConstants.ZHONGWEN_ALPHA_NUMBER_PATTERN) });
        ...
        vu.add("qq", qq, "qq",  new Rule[]{new Digits(11,0)});
        vu.add("face", face, "头像",  new Rule[]{new Length(100)});
        vu.add("province", province, "省",  new Rule[]{new Required()});
        ...
        vu.add("account", account, "账号",  new Rule[]{new Length(15),new NotEmpty(),new Regex(RegexConstants.ALPHA_NUMBER_PATTERN)});
        validStr = vu.validateString();
        if(StringUtil.isNotBlank(validStr)) {
